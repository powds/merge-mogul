...
## JNI references
var _package_manager: JNISingleton
var _context: JNISingleton
...
func load_apps() -> void:
	...
	# Get Android context and package manager via JNI
	var jni_env = JavaSingletonBridge.get_jni_environment()
	if not jni_env:
		push_error("Failed to get JNI environment")
		load_failed.emit("Failed to get JNI environment")
		return
	
	var android_context = JavaSingletonBridge.get_context()
	if not android_context:
		push_error("Failed to get Android context")
		load_failed.emit("Failed to get Android context")
		return
	
	_package_manager = android_context.getPackageManager()
	...
	var apps_array = _package_manager.getInstalledApplications(PackageManager.GET_META_DATA)
	...
	for i in range(apps_array.size()):
		...
		var app_dict = _create_app_dict(app_info)
		if app_dict:
...
func _create_app_dict(app_info) -> Dictionary:
	var app_dict := {
		KEY_PACKAGE_NAME: "",
		KEY_APP_NAME: "",
	...
	if _package_manager and app_dict[KEY_PACKAGE_NAME]:
		var app_name = _package_manager.getApplicationLabel(app_info)
	...
	if "category" in app_info:
		...
		match category_flag:
			...
			ApplicationInfo.CATEGORY_UTILITIES:
				app_dict[KEY_CATEGORY] = CATEGORY_UTILITIES
			_:
				app_dict[KEY_CATEGORY] = CATEGORY_UTILITIES
	
	# Load icon
	app_dict[KEY_ICON] = _load_app_icon(app_info)
	
	return app_dict


## Load app icon from PackageManager
func _load_app_icon(app_info) -> ImageTexture:
	var icon: ImageTexture = null
	
	if _package_manager and app_info:
		var drawable = _package_manager.getApplicationIcon(app_info)
		if drawable:
...
func launch_app_by_package(package_name: String) -> bool:
	...
	var launch_intent = _package_manager.getLaunchIntentForPackage(package_name)
	...
	# Start the activity
	android_context.startActivity(launch_intent)
